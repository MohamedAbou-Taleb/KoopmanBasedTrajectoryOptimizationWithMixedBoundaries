...
% specify the weighting of the soft constraint
weight_soft_constraint = 0.9;
start = tic;
[T_EDMD, cost] = upper_level_pendulum_soft(L_0, L_u, C, x0, T_init, N_opt, opts, weight_soft_constraint);
tspan = linspace(0, T_EDMD, N_opt);
elapsed_time_fmincon = toc(start);
disp(['Elapsed time fmincon: ', num2str(elapsed_time_fmincon)])
dt = tspan(2);
[Z, U_EDMD, cost] = lower_level_pendulum_soft(L_0, L_u, C, x0, N_opt, T_EDMD, weight_soft_constraint);
